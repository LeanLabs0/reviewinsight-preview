...
<th>Last updated</th><th>Source</th></tr></thead>
...
<p class="stamp">Last updated {CAPTURED}</p>
...
def _quote_with_mark(txt: str, ev: str | None, window: int = 38) -> str:
...
def favorite_quotes(v) -> str:
    ...
    def diverse(rows, cap):
        # one per platform first, newest first, then fill with the rest
        seen, first, rest = set(), [], []
        for row in rows:
            portal = row[3]["portal"]
            (rest if portal in seen else first).append(row)
            seen.add(portal)
        return (first + rest)[:cap]

    picks = diverse(pos, 3) + neg[:1]
    if not picks:
